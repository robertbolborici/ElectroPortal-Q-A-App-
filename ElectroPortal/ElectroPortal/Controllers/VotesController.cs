...
namespace ElectroPortal.Controllers
{
    ...
    public class VotesController : ControllerBase
    {
        ...
        [HttpGet("ByAnswerAndUser")]
        [Authorize(AuthenticationSchemes = "Bearer")]
        public async Task<ActionResult<Vote>> GetVoteByAnswerAndUser([FromQuery] string answerId, [FromQuery] string userId)
        {
            var vote = await _context.Votes.FirstOrDefaultAsync(v => v.UserId.Equals(userId) && v.AnswerId.Equals(answerId));
        ...
        public async Task<ActionResult<IEnumerable<UserScoreDto>>> GetLeaderboard()
        {
            ...
            DateTime lastRewardedDate = lastRewardedConfig != null
                                        ? DateTime.Parse(lastRewardedConfig.Value)
                                        : DateTime.MinValue;
            ...
            var userScores = await _context.Answers
                ...
                .Select(group => new
                {
                    ...
                    Score = group.SelectMany(g => g.Votes).Where(v => v.Date >= mondayDate).Sum(v => v.Upvote ? 10 : -10)
                })
                .ToListAsync();
            ...
            if (today.DayOfWeek == DayOfWeek.Monday && today.TimeOfDay < TimeSpan.FromMinutes(1440) && lastRewardedDate < mondayDate)
            {
